...
export const BottomNavMobile: React.FC<BottomNavMobileProps> = ({
  ...
  const navItems = [
    { id: 'all', icon: Calculator, label: 'Tools' },
    { id: 'quantity', icon: Home, label: 'Home' },
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 px-4 py-2 z-40">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <div className="absolute -top-1 w-1 h-1 bg-accent-500 rounded-full" />
              )}
            </button>
          );
        })}
